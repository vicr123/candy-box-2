    ...
    export async function connect() {
        ...
        try {
            ...
            if (slotData.gifting) {
                ...
                await Archipelago.giftManager.fetchGifts();
...
export function findCompatibleSendableItem(traits: GiftTraitType[]) {
    ...
    const surplusOnly = items.filter(item => item.missing.length == 0).flatMap(item => Array(maxSurplus + 1 - item.surplus.length).map(() => item))
    ...
    const weightedItems = items
        .flatMap(item =>
            Array(maxDistance + 1 - (item.missing.length + item.surplus.length)).map(() => item)
        )
